...
(function(root){
 ...
 function useVersion(d,n,request){if(!String(request||'').trim())return {ok:false,error:'Indica la solicitud para controlar duplicados.'};const key=request.trim().toUpperCase();if(d.requestUses[key])return {ok:false,error:'La solicitud ya tiene una PDA generada en este mockup.',existing:d.requestUses[key]};const v=d.versions.find(x=>x.number===n&&x.state==='Publicada');if(!v)return {ok:false,error:'Selecciona una versión publicada y vigente.'};const id='PDA-MOCK-'+Date.now();d.requestUses[key]=id;return {ok:true,id,data:clone(v.data)};}
